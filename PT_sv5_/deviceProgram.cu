...
struct RadiancePRD //?
{
    ...
    unsigned int seed;
    Random       rand;
    //unsigned int coherent_seed; // bm: trying to coherent secondary rays
};
...
static __device__ __forceinline__ float3 SampleLights(const Material& material, float3 albedo, const float etaI, const float etaO, const float3& surfacePos, const float3& surfaceNormal, const float3& wo, Random& rand)
{
    ...
    for (int i = 0; i < kProbeSamples; ++i)
    {
        ...
        if (!occluded)
        {
            // bm: hemisphere? material sampling?
...
//! ------------------------- random seed generator (important)
//!     sv
     unsigned int seed = tea<4>(idx.y * w + idx.x, subframe_index);
     //This seed will produce a noise pattern that vary depending on the pixel position,
     //  and will also change when a new sample will be taken, 
     // but is not animated over time. This way of doing things 
     // is pretty standard.

     //unsigned int seed = tea<4>(idx.x * w + idx.y, subframe_index); //bmm
     
     // bm: coherent test
   //unsigned int coherent_seed = tea<4>(w, subframe_index);

// bm: others, tea<4> however is the best so far, later will come back to this
//      these are just the random number generators
//unsigned int cc = idx.y * w + idx.x;
//unsigned int seed = lcg(cc);
...
//!          // bm tried to merge two random seed, no effect
        //unsigned int lerped_seed = lerp(Random(seed), Random(coherent_seed), 0.9);
        
        prd.seed = seed;
        //prd.seed = lerped_seed;

        prd.rand = Random(seed);
        //prd.rand = Random(lerped_seed);
//--------------------------------------------------------
...
//! sv 
            ...
            {
                //accum_color = clamp(accum_color, make_float3(0.0), make_float3(1.0f)); //10.0f, what is this doing?
                ...
                //accum_color = accum_color_prev + (accum_color - accum_color_prev) * alpha_value;
                //accum_color = accum_color_prev;

                // bm: trying this, do not know what third parameter
                //accum_color = clamp(accum_color_prev, accum_color, make_float3(alpha_value));
...
//! sv
            float3 pprocessingExposureCorrected = float3(accum_color * pow(2.0f, 4.0f)) ; // bm: accum_color is the output final, 2^2 is a good candidat
