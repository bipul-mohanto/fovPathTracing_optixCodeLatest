...
// TIPS: High resolution HDRI’s are difficult to sample and may lead to slow convergence. 
// better off using a low-resolution image for the lighting itself and then compositing in a high resolution background afterwards.
//#define ENV_LIGHT_ON 
...
//#define PER_TIME // bm
//#define PER_FRAME // sv: important, movement for benchmarking

////////////////////////////////////////////////////////////begin GLFW
bool resize_dirty = false;
bool minimized = false;
int2 fbSize = make_int2(3840/4, 2160/4);
...
// why this function? something related to iconification
...
// MIS, environment mapping 
void loadProbe(ProbeData& probe, std::string hdrFile) {
    int resX, resY, channel;
    float4* data = (float4*)stbi_loadf(hdrFile.c_str(), &resX, &resY, &channel, 4); //stbi_loadf loads sRGB (e.g. from BMP, PNG, JPEG)
...
*/
    ...
    ProbeData probe;
    // ambient light
    loadColor(probe, make_float3(5.0f)* make_float3(0.5f)); // light intensity, 10 working fine for sponza
    // K_a * I_a
#else
    //
#endif
...
//! -----------------------------------------------------WINDOW HANDLING 
        ...
        glfwSetWindowIconifyCallback(window, windowIconifyCallback); // bm: what is the use?
