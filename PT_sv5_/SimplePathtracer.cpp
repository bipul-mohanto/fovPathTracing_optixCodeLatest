...
#ifdef FOV_OFF
    //! non-foveation part
    launchParams.frame.subframe_index = 0;//= 0sv; //+=1 bm  //0= accumulation off
...
//!     // periphery
    ...
    launchParams.frame.c = make_uint2(launchParams.frame.c.x, launchParams.frame.c.y);
    launchParams.frame.r_outer = 1000000000;
    launchParams.frame.r_inner = outer_radius;// 
...
{
    const uint32_t    max_trace_depth = 1;//5;//1;
    ...
    // commented for optix 8.0
    //pipelineLinkOptions.debugLevel = OPTIX_COMPILE_DEBUG_LEVEL_MODERATE;//DEFAULT;//FULL;//BM
    ...
    {
        OPTIX_CHECK(optixUtilAccumulateStackSizes(prog_group, &stack_sizes, pipeline)); // bm, added pipeline as third parameter
    }
