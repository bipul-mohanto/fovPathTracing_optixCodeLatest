...
// sanity check: make sure we launch only after first resize is already done:
    ...
    //! non-foveation part
    //! bm: changed to +=1
    launchParams.frame.subframe_index += 1;//sv = 0; // bm: accumulation off
...
//! sv   
    ...
    launchParams.frame.subframe_index = 0;// 0;//0 accumulation important to enable/disable accumulation
...
//    launchParams.frame.offset = make_uint2(0, 0);
    ...
    launchParams.frame.subframe_index = temp_frame;
    launchParams.frame.subframe_index += 1;//++;
